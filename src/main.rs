use std::{net::{TcpStream, ToSocketAddrs}, io::{Write, Read}, time::Duration};
...
fn perform_connection<T: VarIntLen + ToString> (h: &Handshake<T>, t: Duration, short: bool) -> Result<(), ()> {
    ...
    match TcpStream::connect_timeout(&h.to_string().to_socket_addrs().unwrap().last().unwrap(), t) {
        Ok(mut stream) => {
            ...
            stream.set_read_timeout(Some(t)).expect("Invalid read timeout");
            loop {
                let result = match stream.read(&mut buffer) {
                    Ok(size) => {
                        println!("[INFO] Read {} bytes from the server!", size);
                        if short && size > 0 { return Ok(()) }
                        if size > 5 {
...
fn main() {
    ...
    // let h = Handshake::new(Ip::new(141, 144, 197, 26), 25565);
    // println!("[INFO] Trying {}", h.to_string());
    // perform_connection(&h, Duration::from_millis(5000), false);

    for i in 1..=255 {
        let h = Handshake::new(Ip::new(23, 156, 127, i), 25565);
        println!("[INFO] Trying {}", h.to_string());
